  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          const Text('Selecciona método de pago:', style: TextStyle(fontWeight: FontWeight.bold)),
              ...
              child: Column(
                children: [
                  TextFormField(
                    ...
                    validator: (v) => v == null || v.length < 16 ? 'Ingrese 16 dígitos' : null,
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: TextFormField(
                          decoration: const InputDecoration(labelText: 'Expira (MM/AA)'),
                          validator: (v) => v == null || v.length < 5 ? 'MM/AA' : null,
                          onSaved: (v) => _expiry = v ?? '',
                        ),
                      ),
          ...
          if (_method == 'Apple Pay') ...[
            Card(
              ...
              child: ListTile(
                ...
                subtitle: const Text('Confirma el pago con Face ID o Touch ID.'),
              ),
            ),
            const SizedBox(height: 16),
            ElevatedButton(
          ...
          ..._history.map((h) => Card(
                child: ListTile(
                  ...
                  trailing: Text(h['estado']!, style: const TextStyle(color: Colors.green)),
                ),
              )),
        ],
      ),
...
class _PaymentMethodButton extends StatelessWidget {
  ...
  const _PaymentMethodButton({required this.label, required this.icon, required this.selected, required this.onTap});
  @override
  Widget build(BuildContext context) {
    return InkWell(
      onTap: onTap,
